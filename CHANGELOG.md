...
- Backward sync no longer re-executes a block already known as bad, marks the bad chain past the previous marking window on every attempt, and treats an invalid block carried by an exception like `engine_newPayload` does; a block that later imports successfully is dropped from the bad-block caches, and the recorded cause of a bad descendant names the block that actually failed validation.
