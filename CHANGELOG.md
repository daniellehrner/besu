...
- A block carrying a transaction that does not fit the block gas budget now reports that, rather than an EIP-7928 block access list failure. The access list item budget is checked before the block runs, so it pre-empted an error execution would have raised first; it now stands down whenever any transaction in the block is already over the block's budget. At the Amsterdam item cost of 2000 gas, the entries the system contracts alone produce exceed the budget of any block whose gas limit is near the 21000 minimum, so every such block was reported as an access list failure. [#11195](https://github.com/besu-eth/besu/pull/11195)
