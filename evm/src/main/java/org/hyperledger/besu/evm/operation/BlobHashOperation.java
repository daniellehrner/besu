...
public class BlobHashOperation extends AbstractOperation {
  ...
  public OperationResult execute(final MessageFrame frame, final EVM evm) {
    final Word256 indexWord = frame.popStackItem();

    if (indexWord.bitLength() > 31) {
      // Exceeds Integer.MAX_VALUE or negative if interpreted as signed int
      frame.pushStackItem(Word256.ZERO);
      return new OperationResult(3, null);
    }

    final int index = indexWord.toInt();
    if (index < 0) {
      frame.pushStackItem(Word256.ZERO);
      return new OperationResult(3, null);
    }
        ...
        .map(
            hashes -> {
              if (index < hashes.size()) {
                final VersionedHash requested = hashes.get(index);
                frame.pushStackItem(Word256.fromBytes(requested.toBytes().toArrayUnsafe()));
              } else {
                frame.pushStackItem(Word256.ZERO);
              }
              return new OperationResult(3, null);
