...
/** EVM v2 BLOBBASEFEE operation — pushes the blob base fee onto the stack (Cancun+). */
public class BlobBaseFeeOperationV2 extends AbstractFixedCostOperationV2 {

  private static final OperationResult blobBaseFeeSuccess = new OperationResult(2, null);
  ...
  /**
   * Performs the BLOBBASEFEE operation.
   *
  ...
  public static OperationResult staticOperation(final MessageFrame frame, final long[] stack) {
    ...
    frame.setTopV2(StackArithmetic.pushWei(stack, frame.stackTopV2(), frame.getBlobGasPrice()));
    return blobBaseFeeSuccess;
  }
}
